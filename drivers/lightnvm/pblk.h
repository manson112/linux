...
struct line_snapshot
{
	__le32 crc;
	__le64 *line_trans_map;
};
struct pblk_snapshot
{
	struct line_snapshot *buf;
};
									  ...
									  struct line_smeta *smeta)
{
	...
	crc = crc32_le(crc, (unsigned char *)smeta + sizeof(struct line_header) + sizeof(crc),
				   lm->smeta_len -
					   sizeof(struct line_header) - sizeof(crc));
...
/* update snapshot */
static inline u32 pblk_calc_snapshot_crc(struct pblk *pblk,
										 struct line_snapshot *snapshot)
{
	struct pblk_line_meta *lm = &pblk->lm;
	u32 crc = ~(u32)0;

	crc = crc32_le(crc, (unsigned char *)snapshot + sizeof(crc),
				   lm->snapshot_len - sizeof(crc));
	printk("pblk.h[1296]:[pblk_calc_snapshot_crc]:crc = %u\n", crc);
	return crc;
}
									  ...
									  struct line_emeta *emeta)
{
	...
	crc = crc32_le(crc, (unsigned char *)emeta + sizeof(struct line_header) + sizeof(crc),
				   lm->emeta_len[0] -
					   sizeof(struct line_header) - sizeof(crc));
