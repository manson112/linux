...
void *pblk_rb_entries_ref(struct pblk_rb *rb) { return rb->entries; }
...
static int __pblk_rb_update_l2p(struct pblk_rb *rb, unsigned int to_update) {
  ...
  for (i = 0; i < to_update; i++) {
    ...
    pblk_update_map_dev(pblk, w_ctx->lba, w_ctx->ppa, entry->cacheline);
...
unsigned int pblk_rb_read_to_bio(struct pblk_rb *rb, struct nvm_rq *rqd,
  ...
  for (i = 0; i < to_read; i++) {
    ...
    if (bio_add_pc_page(q, bio, page, rb->seg_size, 0) != rb->seg_size) {
...
unsigned int pblk_rb_sync_advance(struct pblk_rb *rb, unsigned int nr_entries) {
  ...
  if (flush_point != EMPTY_ENTRY) {
    ...
    secs_to_flush = pblk_rb_ring_count(flush_point, sync, rb->nr_entries);
...
ssize_t pblk_rb_sysfs(struct pblk_rb *rb, char *buf) {
  ...
  spin_lock_irq(&rb->s_lock);
  list_for_each_entry(c, &pblk->compl_list, list) queued_entries++;
  spin_unlock_irq(&rb->s_lock);

  if (rb->flush_point != EMPTY_ENTRY)
    offset = scnprintf(
        buf, PAGE_SIZE, "%u\t%u\t%u\t%u\t%u\t%u\t%u - %u/%u/%u - %d\n",
        rb->nr_entries, rb->mem, rb->subm, rb->sync, rb->l2p_update,
#ifdef CONFIG_NVM_DEBUG
        atomic_read(&rb->inflight_flush_point),
#else
        0,
#endif
        rb->flush_point, pblk_rb_read_count(rb), pblk_rb_space(rb),
        pblk_rb_flush_point_count(rb), queued_entries);
  else
    offset = scnprintf(
        buf, PAGE_SIZE, "%u\t%u\t%u\t%u\t%u\t%u\tNULL - %u/%u/%u - %d\n",
        rb->nr_entries, rb->mem, rb->subm, rb->sync, rb->l2p_update,
#ifdef CONFIG_NVM_DEBUG
        atomic_read(&rb->inflight_flush_point),
#else
