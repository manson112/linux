...
static ssize_t pblk_sysfs_stats(struct pblk *pblk, char *page) {
  ...
  sz = snprintf(page, PAGE_SIZE,
                "read_failed=%lu, read_high_ecc=%lu, read_empty=%lu, "
                "read_failed_gc=%lu, write_failed=%lu, erase_failed=%lu\n",
...
static ssize_t pblk_sysfs_ppaf(struct pblk *pblk, char *page) {
  ...
  if (geo->version == NVM_OCSSD_SPEC_12) {
    ...
    sz = snprintf(
        ...
        pblk->addrf_len, ppaf->blk_offset, ppaf->blk_len, ppaf->pg_offset,
        ppaf->pg_len, ppaf->lun_offset, ppaf->lun_len, ppaf->ch_offset,
        ppaf->ch_len, ppaf->pln_offset, ppaf->pln_len, ppaf->sec_offset,
        ppaf->sec_len);

    sz += snprintf(
        page + sz, PAGE_SIZE - sz,
        "d:blk:%d/%d,pg:%d/%d,lun:%d/%d,ch:%d/%d,pl:%d/%d,sec:%d/%d\n",
        gppaf->blk_offset, gppaf->blk_len, gppaf->pg_offset, gppaf->pg_len,
        gppaf->lun_offset, gppaf->lun_len, gppaf->ch_offset, gppaf->ch_len,
        gppaf->pln_offset, gppaf->pln_len, gppaf->sec_offset, gppaf->sec_len);
  } else {
...
static ssize_t pblk_sysfs_lines(struct pblk *pblk, char *page) {
  ...
  list_for_each_entry(line, &l_mg->free_list, list) free_line_cnt++;
  spin_unlock(&l_mg->free_lock);

  spin_lock(&l_mg->close_lock);
  list_for_each_entry(line, &l_mg->emeta_list, list) emeta_line_cnt++;
  ...
  list_for_each_entry(line, &l_mg->bad_list, list) bad++;
  list_for_each_entry(line, &l_mg->corrupt_list, list) cor++;
  ...
  if (l_mg->data_line) {
    ...
    map_weight = bitmap_weight(l_mg->data_line->map_bitmap, lm->sec_per_line);
  }
  spin_unlock(&l_mg->free_lock);
  ...
  sz += snprintf(
      ...
      cur_log, nr_free_lines, emeta_line_cnt, meta_weight, closed_line_cnt, bad,
      cor, d_line_cnt, l_line_cnt, l_mg->nr_lines);
...
static ssize_t pblk_sysfs_lines_info(struct pblk *pblk, char *page) {
  ...
  sz += snprintf(page + sz, PAGE_SIZE - sz,
                 ...
                 lm->emeta_sec[0], lm->emeta_bb);
  sz += snprintf(page + sz, PAGE_SIZE - sz,
                 "bitmap lengths: sec:%d, blk:%d, lun:%d\n", lm->sec_bitmap_len,
                 lm->blk_bitmap_len, lm->lun_bitmap_len);
  sz += snprintf(page + sz, PAGE_SIZE - sz,
                 "blk_line:%d, sec_line:%d, sec_blk:%d\n", lm->blk_per_line,
                 lm->sec_per_line, geo->clba);
...
static ssize_t pblk_get_write_amp(u64 user, u64 gc, u64 pad, char *page) {
  ...
  sz = snprintf(page, PAGE_SIZE, "user:%lld gc:%lld pad:%lld WA:", user, gc,
                pad);
...
static ssize_t pblk_sysfs_get_write_amp_trip(struct pblk *pblk, char *page) {
  return pblk_get_write_amp(atomic64_read(&pblk->user_wa) - pblk->user_rst_wa,
                            atomic64_read(&pblk->gc_wa) - pblk->gc_rst_wa,
                            atomic64_read(&pblk->pad_wa) - pblk->pad_rst_wa,
                            page);
}
...
static ssize_t pblk_sysfs_get_padding_dist(struct pblk *pblk, char *page) {
  ...
  for (i = 0; i < buckets; i++) {
    ...
    sz += snprintf(page + sz, PAGE_SIZE - sz, "%d:%lld%% ", i + 1, p);
  }
  sz += snprintf(page + sz, PAGE_SIZE - sz, "\n");
...
static ssize_t pblk_sysfs_stats_debug(struct pblk *pblk, char *page) {
  return snprintf(
      ...
      atomic_long_read(&pblk->req_writes), (u64)atomic64_read(&pblk->nr_flush),
      atomic_long_read(&pblk->padded_writes),
      atomic_long_read(&pblk->padded_wb), atomic_long_read(&pblk->sub_writes),
...
int pblk_sysfs_init(struct gendisk *tdisk) {
  ...
  ret = kobject_init_and_add(&pblk->kobj, &pblk_ktype,
                             kobject_get(&parent_dev->kobj), "%s", "pblk");
  if (ret) {
