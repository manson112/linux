...
void pblk_rl_werr_line_in(struct pblk_rl *rl) { atomic_inc(&rl->werr_lines); }

void pblk_rl_werr_line_out(struct pblk_rl *rl) { atomic_dec(&rl->werr_lines); }
...
void pblk_rl_free_lines_dec(struct pblk_rl *rl, struct pblk_line *line,
  ...
  if (used)
    free_blocks = atomic_sub_return(blk_in_line, &rl->free_user_blocks);
  else
...
int pblk_rl_high_thrs(struct pblk_rl *rl) { return rl->high; }

int pblk_rl_max_io(struct pblk_rl *rl) { return rl->rb_max_io; }
...
void pblk_rl_free(struct pblk_rl *rl) { del_timer(&rl->u_timer); }
