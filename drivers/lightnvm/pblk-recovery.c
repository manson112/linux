...
static void pblk_trans_map_line_add_ordered(struct list_head *head,
                                            struct pblk_line *line) {
  struct pblk_line *t = NULL;

  list_for_each_entry(t, head, list) if (t->trans_map_seq_nr >
                                         line->trans_map_seq_nr) break;
...
struct pblk_line *pblk_recov_l2p(struct pblk *pblk) {
  ...
  snapshot = kmalloc(lm->sec_bitmap_len, GFP_KERNEL);
  ...
  for (i = 0; i < l_mg->nr_lines; i++) {
    ...
    line->trans_map_seq_nr = le64_to_cpu(smeta_buf->trans_map_seq_nr);
    spin_unlock(&line->lock);
    ...
    if (le64_to_cpu(smeta_buf->trans_map_seq_nr) != 0) {
      pblk_trans_map_line_add_ordered(&trans_map_line_list, line);
      pr_debug("pblk: recovering trans map line %d, trans_map_seq:%llu\n",
               line->id, smeta_buf->trans_map_seq_nr);
    } else {
  ...
  lba = 0;
  list_for_each_entry_safe(line, tline, &trans_map_line_list, list) {
    ...
    ppa.ppa = line->snapshot->buf->snapshot[lba++];
